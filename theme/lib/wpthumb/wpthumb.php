...
function wpthumb_errors() {
	...
	if ( file_exists( $dir_upload ) && !is_writable( $dir_upload ) )
		echo '<div id="wpthumb-warning" class="updated fade"><p><strong>' . __( 'WPThumb has detected a problem.', 'wpthumb', 'fortytwo-dev' ) . '</strong> ' . sprintf( __( 'The directory <code>%s</code> is not writable.', 'wpthumb', 'fortytwo-dev' ), $dir_upload ) . '</p></div>';
