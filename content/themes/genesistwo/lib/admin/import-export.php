...
class Genesis_Admin_Import_Export extends Genesis_Admin_Basic {
	...
	protected function export_checkboxes() {
		...
		foreach ( $options as $name => $args ) {
			...
			printf( '<label for="genesis-export-%1$s"><p><input id="genesis-export-%1$s" name="genesis-export[%1$s]" type="checkbox" value="1" /> %2$s</label></p>', esc_attr( $name ), esc_html( $args['label'] ) );
